...
- result: both slices accepted by the operator on 2026-06-13 in one card, "ytgrab core: the resilient downloader" (commit b0eca9d, "operator accepts"). The whole click-to-run experience is built and accepted; the channel downloads on the operator's machine once they install ffmpeg and update yt-dlp — their call, those touch the machine, not the project. Check met.
- state: folded
- since: 2026-06-13
- of: ytgrab
...
- result: built this session — ytgrab.py + config.example.json + README, py_compile clean. Not run here (it downloads gigabytes over the network — the operator's to trigger; that is the acceptance try). Hardened in a later session: a preflight now guards ffmpeg, which yt-dlp needs to merge bestvideo+bestaudio into mp4 — an environment probe found ffmpeg absent, and with ignoreerrors set a missing ffmpeg would have skipped every video silently; it now exits with an install hint before any download. Hardened again (2026-06-13) after reading the operator's own try: the run log showed two clicks that logged "done — every available video is downloaded" in one second having saved nothing (archive empty, no files). Two silent-skip defects fixed — a bare `@handle` (which the launcher invites) was rejected by yt-dlp as "not a valid URL" and swallowed by ignoreerrors, now normalized to the channel's /videos URL; and the unconditional "done" now reports the truth (saved N / up to date / found-but-saved-none / channel-yielded-nothing, the last two non-zero exit). The empty extraction's root cause is the operator's yt-dlp 2024.04.09 (HTTP 400 on a video, 0 items on every channel) — environment, surfaced on the card. Accepted by the operator 2026-06-13 (b0eca9d).
- state: done
